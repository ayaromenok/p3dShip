...
module sideFrontBottomCyl_4 (size=200, height=200, px=0, py=0, pz=0, rx=0, ry=0, rz=0, topSupportRound=false){
    translate([px, py, pz])
    rotate([rx, ry, rz]) {
        size_2 = size/2;
        size_4 = size/4;
        ...
        difference(){
            union(){
                translate([size_2,-size,-height_2])    
                linear_extrude(height, $fn=200)
                scale([3,3,1])
                    difference(){
                        import("../dxf/profileHullFront.dxf"); //size 200x100
                        offset(r=-0.8){
                            import("../dxf/profileHullFront.dxf"); //size 200x100
                        }//offset
                    }//diff
            ...
            yCube(LDiameter,size+LDiameter,height+1,size_2+LRadius);            
            yCube(size*10,size*10,height+1,-size_2-size*5);
            yCube(size+LDiameter,LDiameter,height+1,0,size_2+LRadius);
            yCube(size*10,size*10,height+1,0,-size_2-size*5);            
        }//diff
